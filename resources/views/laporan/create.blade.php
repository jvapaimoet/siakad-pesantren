...
<head>
    ...
    <title>SIPES - Buat Laporan</title>
    ...
    <main class="flex-1 flex flex-col overflow-y-auto">
        <header class="bg-white px-8 py-4 flex justify-between items-center shadow-sm">
            <div>
                ...
                <p class="text-sm text-gray-500">Halaman Laporan Sistem</p>
            </div>
        </header>
        ...
        <section class="p-8 space-y-6">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h3 class="text-xl font-bold text-gray-800">Cetak Laporan Baru</h3>
                    <p class="text-sm text-gray-500 mt-1">Pilih kriteria laporan yang ingin dicetak atau diexport ke sistem.</p>
                </div>
                <a href="{{ route('laporan.index') }}" class="text-sm font-medium text-gray-600 hover:text-gray-900 bg-white border px-4 py-2 rounded-lg shadow-sm flex items-center space-x-2">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
                <form action="{{ route('laporan.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="jenis_laporan" id="jenis_laporan" required class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500 shadow-sm p-2 bg-gray-50 border">
                            <option value="">-- Pilih Laporan --</option>
                            <option value="santri" @selected(old('jenis_laporan') === 'santri')>Laporan Data Keseluruhan Santri</option>
                            <option value="ustadz" @selected(old('jenis_laporan') === 'ustadz')>Laporan Data Ustadz Pengajar</option>
                            <option value="absen" @selected(old('jenis_laporan') === 'absen')>Laporan Presensi Bulanan</option>
                        </select>
                        @error('jenis_laporan') <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="tanggal" class="block text-sm font-semibold text-gray-700 mb-1">Tanggal Laporan</label>
                        <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', now()->toDateString()) }}" class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500 shadow-sm p-2 bg-gray-50 border">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3" class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500 shadow-sm p-2 bg-gray-50 border" placeholder="Catatan tambahan laporan jika diperlukan">{{ old('deskripsi') }}</textarea>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        <button type="submit" class="px-5 py-2 rounded-lg text-sm font-medium text-white transition flex items-center space-x-2" style="background-color: #046A4E;">
                            ...
                            <span>Proses & Cetak Laporan</span>
                        </button>
                    </div>
                </form>
            </div>
